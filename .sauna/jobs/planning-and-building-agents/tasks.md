...
- [x] Wire hooks into `runJobPipeline` — `JobPipelineDeps` extended with optional `hooks`, `runHooks`, `hookCwd`, `maxHookRetries`, and `onHookFailure`; `runJobPipeline()` passes all hook fields through to `runUntilDone`; CLI `main()` calls `loadHooks(args.codebase)` and passes result with `runHooks` and `hookCwd` to `runJobPipeline`; 4 new tests in `job-pipeline.test.ts` verify hooks are forwarded, empty hooks are skipped, no-hooks defaults work, and `onHookFailure` is passed through (spec: builder-hooks, end-to-end integration)
