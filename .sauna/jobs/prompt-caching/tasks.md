...
- [ ] Convert `createDiscoveryAgent` instructions to instruction object; apply dynamic output-directory suffix to `content` before wrapping (spec: agent-instructions-caching)
- [ ] Convert `createPlanningAgent` instructions to instruction object; apply `${JOB_ID}` substitution to `content` before wrapping (spec: agent-instructions-caching)
...
- [ ] Update `agent-definitions.test.ts` — `getInstructions()` now returns an instruction object; adapt `toContain` assertions to check the `content` field (spec: agent-instructions-caching)
- [ ] Update `cli.test.ts` — discovery agent tests that call `getInstructions()` and assert plain strings must handle the instruction object shape (spec: agent-instructions-caching) (note: research agent tests already updated)
