...
- [x] Replace 2 remaining `any` types (`MastraOnFinishCallback`) — derived `OnFinishCallback` type from `Agent['stream']` parameters using `NonNullable<Parameters<Agent["stream"]>[1]>["onFinish"]`; replaced `any` in `session-runner.ts:11,19` and `cli.ts:50`; exported `OnFinishCallback` from `src/index.ts`; 0 tsc errors, 101 tests passing (99 pass, 2 skip)
