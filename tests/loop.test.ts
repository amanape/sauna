...
describe("runLoop", () => {
  ...
  test("--count 3: runs exactly 3 iterations with headers", async () => {
    ...
    await runLoop({ forever: false, count: 3 }, factory.run, (s) =>
      output.push(s),
    );
  ...
  test("--count 1: runs once with header", async () => {
    ...
    await runLoop({ forever: false, count: 1 }, factory.run, (s) =>
      output.push(s),
    );
  ...
  test("formatting state resets between loop iterations", async () => {
    ...
    const beforeIter2 = joined.substring(
      Math.max(0, iter2Start - 3),
      iter2Start,
    );
